...
class PlanningCaptureAgent(CaptureAgent):
  ...
  def max_value(self,gameState, depth):
    ...
    for action in gameState.getLegalActions(self.index):
        ...
        successor = gameState.generateSuccessor(self.index,action)
        #minimax search이므로 exp-value에서 나온 값들 중 최댓값을 구한다.
        #이때 min-value는 첫번째 ghost부터 해야하므로 index를 self.opponents[0]으로 주고, depth는 그대로 넘겨준다.
        v = max(v, self.exp_value(successor,self.opponents[0],depth))
    #v를 반환한다.
    return v
        
  #chance노드에서의 행동에 대한 함수 exp-value를 정의한다.
  #이때 ghost마다 각각의 index가 있으므로 index도 같이 받는다.
  def exp_value(self,gameState,index,depth):
    #v를 0으로 선언한다.
    v=0
    #가능한 행동들에 대해서 노드를 평가한다.
    for action in gameState.getLegalActions(index):
      #가능한 actions 중 action을 취했을 때 도달하는 상태를 successor이라고 하자.
      successor = gameState.generateSuccessor(index,action)
      #만약 현재 ghost의 index가 마지막 index라면 다음에는 pacman에 대해 max_search가 실행되야한다.
      if index==self.opponents[1]:
        #v에 다음 노드를 max_value로 평가한 값을 더해준다.
        #이때 한 번의 탐색이 끝났으므로 depth를 1증가시킨다.
        v = v+ self.max_value(successor,depth+1) #beacuse of last search
      #만약 다음 탐색이 ghost에 대한 것일 경우
      else:
        #v에 다음 노드를 exp_value로 평가한 값을 더해준다.
        #이때 다음 ghost의 index를 넘겨준다.
        v = v+ self.exp_value(successor,self.opponents[1],depth) #because of not last seach

    #v를 가능한 행동의 개수로 나눈 값(확률)을 반환한다.
  ...
  def getAction(self,gameState):  
    ...
    #invader과의 거리가 가장 가까워지는 action을 구할 것이다.
    bestDist = float('inf')
    #만약 상대 ghost와 거리가 1이고 scaredTimer가 0이면 ghost와가까운 곳으로 간다.
    if minIDist==1 and myState.scaredTimer<=0:
      #가능한 action들에 대해
      for action in gameState.getLegalActions(self.index):
        #ghost와의 거리를 구한다.
        dist = self.getMazeDistance(pos,invaders[minIndex].getPosition())
        #최소값을 구하고 그때의 행동을 next_action이라고 하자.
        if dist<bestDist:
          next_action = action
          bestDist = dist
      #next_action을 반환한다.
    ...
    #pacman이 움직여야 할 방향을 expectimax_action이라고 선언하고 0으로 초기화하였다.
    expectimax_action = 0

    #expectimax_value의 최댓값을 구하고 그 값에 해당하는 노드로 이동할 것이다.
    #이때 최댓값을 구해야하므로 ecpectimax_value를 -infinity로 초기화하였다.
    expectimax_value = float('-inf')
    #depth = 0에서 pacman의 다음 노드중에서 값이 가장 높은 것을 찾는다.
    for action in gameState.getLegalActions(self.index):
        #가능한 actions 중 action을 취했을 때 도달하는 상태를 successor이라고 하자.
        successor = gameState.generateSuccessor(self.index,action)
        #팩맨의 다음 노드는 1번 ghost이므로 self.opponents[0] 인덱스를 가지는 ghost 대해 exp_value를 구한다.
        #구한 값을 val이라고 하자. 
        val = self.exp_value(successor, self.opponents[0], 0)
        #val들 중 최댓값을 구하고, 그때의 action을 expectimax_action에 저장한다.
        if expectimax_value <val:
            expectimax_action = action
            expectimax_value = val
    #expectimax_action을 반환한다.  
...
class attack(PlanningCaptureAgent):
  ...
  def getFeatures(self, gameState):
    ...
    if minGDist<=1:
      ...
      #탐색중인 agent(나)가 pacman이라면
        ...
        if defenders[minIndex].scaredTimer>4:
          ...
          #distanceToGhost features에 minGDist를 저장한다.
          features['distanceToGhost'] = minGDist
          #distanceToFood features에 10*minFDist를 저장하여 음식과의 거리에 가중치를 더 주었다.
          #ghost와의 거리보다 음식을 먹는 것에 집중하기 위함이다.
          features['distanceToFood'] = 10*minFDist
    ...
    return features['noWay']*1+features['goToHome']*(-76)+features['havingFood']*190+features['canCatch']*1+features['distanceToFood']*(-139.5)+features['distanceToGhost']*80+features['numOfGhost']*(-15)+features['nextScore']*8000+features['successorScore']*(-3200)+features['leftCapsules']*(-3000)+features['distanceToCapsule']*(-219)
