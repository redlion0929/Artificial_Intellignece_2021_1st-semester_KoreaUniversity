...
def createTeam(firstIndex, secondIndex, isRed,
               first = 'all_rounder_agent', second = 'all_rounder_agent'):
  """
...
class PlanningCaptureAgent(CaptureAgent):
  ...
  #min노드에서의 행동에 대한 함수(min_value)를 정의한다.
...
#all_rounder_agent 정의
class all_rounder_agent(PlanningCaptureAgent):
  """
  ...
  def getFeatures(self, gameState):
    ...
    #현재 탐색 대상인 agent(나)말고 다른 같은 팀원의 index를 구한다.
    #other_index를 선언하고 0으로 초기화한다.
    other_index = 0
    #같은 팀의 index를 검사하면서
    for i in self.teams:
      #index가 나의 index와 같지 않다면
      if i!=self.index:
        #그 index를 other_index라고 하자.
        other_index = i

    #침입자가 0명이거나 남아있는 음식의 개수가 14개보다 크거나 같을 경우 모두 공격에 참여한다.
    if len(invaders)==0 or len(self.getFoodYouAreDefending(gameState).asList())>14:
      ...
      #otherIndex features에 현재 탐색 대상인 agent(나)와 같은 팀원 사이의 거리를 저장한다.
      features['otherIndex'] = self.getMazeDistance(pos, gameState.getAgentPosition(other_index))
      ...
      return features['otherIndex']*20+features['noWay']*1+features['canCatchN']*100+features['goToHome']*(-78.5)+features['havingFood']*190+features['wall']*1+features['canCatch']*1+features['distanceToFood']*(-139.5)+features['distanceToGhost']*80+features['numOfGhost']*(-15)+features['nextScore']*8000+features['successorScore']*(-3200)+features['leftCapsules']*(-3000)+features['distanceToCapsule']*(-218.5)

    #침입자가 한 명이고 남아있는 음식의 개수가 14개보다 작을 경우 
    elif len(invaders)==1 and len(self.getFoodYouAreDefending(gameState).asList())<=14:
      #나의 index가 self.tems[0]이면 수비에 참여
      if self.index==self.teams[0]:
        #Counter로 features를 선언한다.
        features = util.Counter()
        #현재 탐색 대상인 agent(나)의 위치를 pos에 저장한다.
        pos = gameState.getAgentPosition(self.index)
        ...
        if len(invaders)>0:
          ...
          if minIDist<=1:
            ...
            #my state가 scared가 아니면
              ...
              features['canCaught'] = 1000*(3-minIDist)
          #만약 invader과의 최소거리가 1보다 작거나 같으면
          if minIDist <= 1:
    ...
    #내가 경계에서 가장 가깝지 않았으면 공격을 계속한다.
    #이때 같은 팀원은 수비를 하러 갔으므로 otherIndex는 고려하지 않는다.
      else:
        #Counter로 features를 선언한다.
        features = util.Counter()
        ...
        #위에서 구한 features에 적절한 가중치를 주어 값을 반환한다.
        #이때 팀원은 수비를 하러 갔으므로 otherIndex는 고려하지 않는다.
        return features['noWay']*1+features['canCatchN']*100+features['goToHome']*(-78.5)+features['havingFood']*190+features['wall']*1+features['canCatch']*1+features['distanceToFood']*(-139.5)+features['distanceToGhost']*80+features['numOfGhost']*(-15)+features['nextScore']*8000+features['successorScore']*(-3200)+features['leftCapsules']*(-3000)+features['distanceToCapsule']*(-218.5)

        
    #침입자가 2명일 경우 음식보다는 상대 pacman을 따라다닌다. 둘이 따라다니면 잡을 확률이 높기 때문이다.    
    else:
      #Counter로 features를 선언한다.
      features = util.Counter()
      #현재 탐색 대상인 agent(나)의 위치를 pos에 저장한다.
      ...
      #막아야 할 capsule에 대한 정보를 capsuleList에 저장한다.
      capsuleList = self.getCapsulesYouAreDefending(gameState)
      #otherIndex features에 탐색중인 agent(나)와 팀원 사이의 거리를 저장한다.
      features['otherIndex'] = self.getMazeDistance(pos, gameState.getAgentPosition(other_index))
      ...
      #위에서 구한 features에 적절한 가중치를 주어 값을 반환한다
      return features['catchP']+features['otherIndex']*20+features['onDefense']*1000+features['canCaught']*100+features['distanceToFood']*(-150) +features['distanceToInvador']*(-500)+features['numOfInvadors']*(-5000)+features['nextScore']*500+features['leftCapsules']*2000+features['distanceToCapsule']*(-200)